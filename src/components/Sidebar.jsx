...
const Sidebar = () => {
    ...
    const roleFilteredGroups = MENU_GROUPS
    ...
    useEffect(() => {
        ...
        roleFilteredGroups.forEach(group => {
    ...
    const filteredMenu = roleFilteredGroups.map(group => {
    ...
    useEffect(() => {
        if (searchText) {
            const allOpen = {};
            roleFilteredGroups.forEach(g => allOpen[g.title] = true);
        ...
        >
            ...
            <Box sx={{ p: 2, pb: 1 }}>
                <TextField
                    ...
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 3, backgroundColor: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' } }}
                    InputProps={{
                        ...
                        return (
                            <React.Fragment key={index}>
                                <ListItemButton
                                    ...
                                    sx={{
                                        borderRadius: 2, mx: 1.5, my: 0.5,
                                        color: (isOpen || isGroupActive) ? groupThemeColor : 'text.primary',
                                        '&:hover': { bgcolor: (isOpen || isGroupActive) ? `${groupThemeColor}0D` : 'rgba(0,0,0,0.04)' },
                                        transition: 'all 0.3s ease'
                                    }}
                                >
                                    {groupIcons[group.title] && React.cloneElement(groupIcons[group.title], {
                                        sx: { mr: 2, fontSize: '1.2rem', color: (isOpen || isGroupActive) ? groupThemeColor : '#757575', transition: 'color 0.3s' }
                                    })}
                                    <ListItemText primary={group.title}
                                        primaryTypographyProps={{ fontWeight: (isOpen || isGroupActive) ? 700 : 500, fontSize: '0.9rem' }}
                                    />
                                    {isOpen ? <ExpandLess sx={{ color: groupThemeColor }} /> : <ExpandMore sx={{ color: 'action.disabled' }} />}
                                </ListItemButton>
                                        ...
                                        {group.items.map((item) => {
                                            ...
                                            return (
                                                <ListItemButton
                                                    ...
                                                    to={`/${item.path}`}
                                                    sx={{
                                                        pl: 6, borderRadius: 2, mx: 1.5, my: 0.25,
                                                        ...
                                                        '&::before': isActive ? {
                                                            ...
                                                            width: '4px', backgroundColor: groupThemeColor,
                                                        } : {},
                                                        '&:hover': {
                                                            bgcolor: isActive ? `${groupThemeColor}25` : 'rgba(0,0,0,0.03)',
                                                            color: groupThemeColor,
                                                            '& .MuiSvgIcon-root': { color: groupThemeColor }
                                                        },
                                                ...
                                                >
                                                    {itemIcons[item.path]
                                                        ? React.cloneElement(itemIcons[item.path], {
                                                            sx: { mr: 2, fontSize: '1.1rem', color: isActive ? groupThemeColor : '#9e9e9e', transition: 'color 0.2s' }
                                                        })
