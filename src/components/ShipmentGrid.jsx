...
import {
    Grid, Card, CardContent, Box, Chip, IconButton, Typography,
            ...
            <CardContent sx={{
                ...
                <Box sx={{ flexGrow: 1 }}>
                    <Box sx={{ display: 'flex', gap: 1.8, mb: 2, minHeight: '110px', alignItems: 'stretch' }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 0.5 }}>
                            ...
                            <Box sx={{
                                ...
                                >
                                    {isHistoryExpanded ? <RemoveCircleOutline sx={{ fontSize: 14, color: mainColor }} /> : <AddCircleOutline sx={{ fontSize: 14, color: mainColor }} />}
                                </IconButton>
                            </Box>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                            ...
                            <Collapse in={isHistoryExpanded}>
                                <Box sx={{ my: 1.5, pl: 1.5, borderLeft: `2px solid ${alpha(mainColor, 0.1)}`, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                    {history.length > 0 ? (
                                        history.filter(step => step.locationName !== s.originLocationName && step.locationName !== s.destinationLocationName)
                                            .map((step, idx) => (
                                                <Box key={idx} sx={{ position: 'relative', mb: 1.5 }}>
                                                    <Box sx={{ position: 'absolute', left: -17, top: 6, width: 6, height: 6, borderRadius: '50%', bgcolor: alpha(mainColor, 0.4) }} />
                                        ...
                                        <Typography variant="caption" color="text.disabled" sx={{ fontStyle: 'italic', fontSize: '0.7rem' }}>Транзитних пунктів не зафіксовано</Typography>
                                    )}
                                </Box>
                            </Collapse>
                    ...
                    <Box sx={{ mb: 2, display: 'flex', flexDirection: 'column', gap: 0.8 }}>
                        ...
                        {s.issuedAt && (
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: alpha(theme.palette.success.main, 0.05), p: 0.5, px: 1, borderRadius: 1.5 }}>
                    ...
                    <Box sx={{ mt: 1.5, pt: 1, borderTop: '1px dashed #ddd' }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
                            {(() => {
                                ...
                                return (
                                    <Chip icon={status.icon} label={status.label} size="small"
                                        sx={{ height: 20, fontSize: '0.65rem', fontWeight: 800, bgcolor: alpha(status.color, 0.1), color: status.color, border: `1px solid ${alpha(status.color, 0.2)}` }}
                                    />
                                );
                            })()}
                            <Button size="small" onClick={() => onToggleFinance(s.id)} endIcon={isFinanceExpanded ? <ExpandLess /> : <ExpandMore />} sx={{ fontSize: '0.65rem', fontWeight: 700, p: 0, minWidth: 'auto', textTransform: 'none', color: mainColor }}>
                        ...
                        <Collapse in={isFinanceExpanded}>
                            <Box sx={{ mt: 1, display: 'flex', flexDirection: 'column', gap: 0.8 }}>
                                {s.payments?.map(p => (
                                    <Box key={p.id} sx={{ p: 0.8, borderRadius: 1, bgcolor: alpha(theme.palette.success.main, 0.03), borderLeft: `2px solid ${theme.palette.success.main}`, display: 'flex', justifyContent: 'space-between' }}>
                                        <Box><Typography variant="caption" sx={{ fontWeight: 800, display: 'block', lineHeight: 1 }}>{p.amount} ₴</Typography><Typography variant="caption" sx={{ fontSize: '0.55rem', color: 'text.secondary' }}>{p.paymentTypeName}</Typography></Box>
                ...
                <Box sx={{ mt: 'auto', pt: 2 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 1 }}>
                        <Box>
                            <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600, display: 'block' }}>Загальна вартість:</Typography>
                            {s.remainingAmount > 0 && <Typography variant="caption" sx={{ color: 'error.main', fontWeight: 800, fontSize: '0.7rem' }}>Борг: {s.remainingAmount} ₴</Typography>}
                        </Box>
                        <Typography variant="h6" fontWeight="900" sx={{ color: s.isFullyPaid ? 'success.main' : 'text.primary', lineHeight: 1 }}>{s.totalPrice} ₴</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Chip label={s.shipmentStatusName} size="small" variant="outlined" sx={{ height: 22, fontSize: '0.65rem', fontWeight: 800, borderColor: statusColor, color: statusColor, bgcolor: alpha(statusColor, 0.08) }} />
